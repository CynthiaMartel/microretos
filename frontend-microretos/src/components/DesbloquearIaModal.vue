...
async function verificarCodigo() {
  if (!codigoInput.value || verificando.value) return
  verificando.value = true
  errorAcceso.value = ''
  try {
    await api.post(`/equipo/${props.token}/ia/verificar-codigo`, { codigo: codigoInput.value }, { skipAuthRedirect: true })
    exitoso.value = true
    setTimeout(() => emit('desbloqueado'), 900)
  } catch (e) {
    errorAcceso.value = e.response?.data?.message || 'Código incorrecto. Inténtalo de nuevo.'
    codigoInput.value = ''
  } finally {
  ...
  >
    <div class="relative bg-[#1a2332] border border-white/10 rounded-[2rem]
      ...
      <div class="flex justify-center mb-6">
        <div class="w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-500"
          ...
          <Transition name="icon-swap" mode="out-in">
            ...
            <svg v-else key="lock" class="w-8 h-8 text-violet-400"
                 fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <h1 class="text-2xl font-black text-center tracking-tight mb-1">
        ✨ Sugerir con <span class="text-violet-400">IA</span>
      </h1>
      ...
      <Transition name="fade">
        <div v-if="!exitoso">
          <p class="text-white/40 text-sm text-center mt-2 mb-8">
            Vuestro docente reparte un código para activar las sugerencias de IA. Pídeselo si no lo tenéis.
          </p>

          <form @submit.prevent="verificarCodigo" class="space-y-4">
            <div>
            ...
            <button
              type="submit"
              :disabled="verificando || !codigoInput"
              class="w-full py-3 rounded-full bg-violet-600 text-white font-black text-xs
            ...
            >
              ...
              {{ verificando ? 'Comprobando...' : 'Desbloquear' }}
            </button>
